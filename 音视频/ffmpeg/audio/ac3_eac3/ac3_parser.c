...
int avpriv_ac3_parse_header(GetBitContext *gbc, AC3HeaderInfo **phdr)
{
    ...
    hdr->bitstream_id = show_bits_long(gbc, 29) & 0x1F;     //crc1:16 bits,fscod:2 bits,frmsizecod:6 bits,bsid:5 bits = 29 bits
    ...
    if(hdr->bitstream_id <= 10) {
        /* Normal AC-3 */
        hdr->crc1 = get_bits(gbc, 16);      //syncinfo->crc1:16 bits
        hdr->sr_code = get_bits(gbc, 2);    //syncinfo->fscod:2 bits
        if(hdr->sr_code == 3)
            return AAC_AC3_PARSE_ERROR_SAMPLE_RATE;

        frame_size_code = get_bits(gbc, 6); //syncinfo->frmsizecod:6 bits
        if(frame_size_code > 37)
            return AAC_AC3_PARSE_ERROR_FRAME_SIZE;

        skip_bits(gbc, 5); // skip bsid, already got it

        hdr->bitstream_mode = get_bits(gbc, 3);     //bsi->bsmod:3 bits  
        hdr->channel_mode = get_bits(gbc, 3);       //bsi->acmod:3 bits
